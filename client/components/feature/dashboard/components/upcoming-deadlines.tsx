...
export const UpcomingDeadlines = React.memo(function UpcomingDeadlines() {
  ...
  return (
    <Card className="glass border-white/40 hover:border-warning-400 hover:shadow-2xl transition-all duration-300">
      ...
      <CardContent className="p-6 pt-0">
        <div className="space-y-3">
          {upcomingTasks.map((task) => (
            <div
              key={task.id}
              className="flex items-start gap-3 p-3 rounded-xl glass-light shadow-sm hover:glass-light transition-all duration-300 border border-neutral-200 hover:border-neutral-300 group cursor-pointer"
            >
              <div className={cn(
                "p-2 rounded-lg mt-0.5",
                task.isOverdue ? "bg-danger-500/10" : "glass-light"
              )}>
                {task.isOverdue ? (
                  <AlertCircle className="h-4 w-4 text-danger-400" />
                ) : (
                  <Calendar className="h-4 w-4 text-neutral-600 group-hover:text-neutral-300" />
                )}
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2 mb-1">
                  <p className="text-sm font-medium text-neutral-900 group-hover:text-primary-400 transition-colors duration-300 line-clamp-1">
                    {task.title}
                  </p>
